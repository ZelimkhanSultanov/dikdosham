...
function readyDoc() {
	jQuery("#dosh_block #translate_btn").click(translator);
...
function translator() {
	...
	var errorNoWord = '<span id="error_no_word">Дош чу ца йаздина.</span>';
	var errorTranslate = '<span id="error_translate">Дошамаш чохь и дош дац.</span>';
 							...
 							for (var j=0; j<dict.length; j++) {
 								...
 								if (mass[dict_table]) {
 									...
 									for (var i=0; i<mass[dict_table].length; i++) {
 										...
 										jQuery("#result").append('<div class="slovo">'+slovo+' <span class="open_perevod">гочде</span> </div><div class="perevod"> '+perevod+'</div>');
 									}
 								}	
 							}
